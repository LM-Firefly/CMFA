...
androidComponents.onVariants { variant ->
    ...
    abiMap.forEach { (abi, pair) ->
        ...
        // 注意：每个 copy 任务必须拥有互不重叠的 outputDir，
        // 否则在增量构建下会出现任务输出互相污染，mergeJni 可能读到旧 so。
        val jniTaskOutDirProvider = layout.buildDirectory.dir("generated/jniLibs/${variant.name}/${abi}")
        val copyTask = tasks.register("copyGoLib${variantCap}${goAbi}", CopyGoLibsTask::class.java) {
            dependsOn(t)
            val soFileProvider = outDirProvider.map { it.resolve("libclash.so") }
            inputFile.set(layout.file(soFileProvider))
            outputDir.set(jniTaskOutDirProvider)
            this.abi.set(abi)
        }
