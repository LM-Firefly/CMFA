...
// 全局版本号变量，便于同步
val appVersionName = "2.11.31"
val appVersionCode = 211031
plugins {
...
subprojects {
    ...
    fun queryConfigProperty(key: String): String? {
    ...
    if (isApp) {
        extensions.configure<ApplicationExtension> {
            ...
            defaultConfig {
                val customApplicationId = queryConfigProperty("custom.application.id")
                applicationId = customApplicationId?.takeIf { it.isNotBlank() } ?: "com.github.lm_firefly.cmfa"
                minSdk = 23
                targetSdk = 36
            ...
            compileSdk = 36
                packaging {
                    ...
                    jniLibs {
                        keepDebugSymbols += setOf("**/libclash.so", "**/libbridge.so")
                    }
                }
                flavorDimensions += "feature"
                val removeSuffix = queryConfigProperty("remove.suffix")?.toBoolean() == true
                productFlavors {
                    create("alpha") {
                        dimension = "feature"
                        if (!removeSuffix) {
...
tasks.wrapper {
    ...
    distributionSha256Sum = URI.create("${distributionUrl}.sha256").toURL().openStream()
